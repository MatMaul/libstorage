...
import (
	...
	// load the  driver
	openstack "github.com/emccode/libstorage/drivers/storage/openstack"
	_ "github.com/emccode/libstorage/drivers/storage/openstack/executor"
	_ "github.com/emccode/libstorage/drivers/storage/openstack/storage"
)
...
func TestInstanceID(t *testing.T) { //PASSES lowercase hidden for testing other stuff
	...
	executor, err := registry.NewStorageExecutor(driverName)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := executor.Init(ctx, configR); err != nil {
		t.Fatal(err)
	}
	...
	iid, err := executor.InstanceID(ctx, configR)
	if err != nil {
		t.Fatal(err)
	}
...
func volumeCreate(
	...
	reply, err := client.API().VolumeCreate(nil, driverName, volumeCreateRequest)
...
func volumeInspect(
	...
	reply, err := client.API().VolumeInspect(nil, driverName, volumeID, false)
	assert.NoError(t, err)
...
func volumeInspectAttached(
	...
	log.WithField("volumeID", volumeID).Info("inspecting volume")
	reply, err := client.API().VolumeInspect(nil, driverName, volumeID, true)
	assert.NoError(t, err)
...
func snapshotInspect(
	...
	reply, err := client.API().SnapshotInspect(nil, driverName, snapshotID)
	assert.NoError(t, err)
...
func snapshotByName(
	...
	log.WithField("snapshotName", snapshotName).Info("get snapshot by name")
	snapshots, err := client.API().Snapshots(nil)
	assert.NoError(t, err)
	if err != nil {
		t.FailNow()
	}
	assert.Contains(t, snapshots, driverName)
	for _, vol := range snapshots[driverName] {
...
func volumeSnapshot(
	...
	reply, err := client.API().VolumeSnapshot(nil, driverName,
...
func snapshotCopy(
	...
	reply, err := client.API().SnapshotCopy(nil, driverName,
...
func volumeCreateFromSnapshot(
	...
	reply, err := client.API().VolumeCreateFromSnapshot(nil,
		driverName, snapshotID, volumeCreateRequest)
	assert.NoError(t, err)
	if err != nil {
		t.FailNow()
...
func volumeCopy(
	...
	reply, err := client.API().VolumeCopy(nil,
		driverName, volumeID, volumeCopyRequest)
	assert.NoError(t, err)
	if err != nil {
		t.FailNow()
